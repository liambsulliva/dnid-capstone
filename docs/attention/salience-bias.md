...
1. **Size**: Making an element larger than the surrounding elements.
2. **Color**: Using a color that is more visually striking than the surrounding elements.
3. **Motion**: Using animation to draw the eye to the element.

While not particularly unique on its own, achieving salience means drawing the user's attention to something important. It is at the backbone of many design patterns featured elsewhere in this wiki, including (but not limited to) the [centre stage effect](/docs/anchoring/centre-stage-effect) and [red dot effect](/docs/attention/red-dot-effect).
